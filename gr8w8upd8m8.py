...
class Wiiboard:
        ...
        while self.status == "Connected" and not self.processor.done:
            ...
            if intype == INPUT_STATUS:
                ...
                if self.calibrationRequested:
                    packet_length = (int(str(data[4]).encode("hex"), 16) / 16 + 1)
                    self.calibration_parser(data[7:(7 + packet_length)])
    ...
    def create_board_event(self, bytes):
        ...
        raw_tr = (int(bytes[0].encode("hex"), 16) << 8) + int(bytes[1].encode("hex"), 16)
        raw_br = (int(bytes[2].encode("hex"), 16) << 8) + int(bytes[3].encode("hex"), 16)
        raw_tl = (int(bytes[4].encode("hex"), 16) << 8) + int(bytes[5].encode("hex"), 16)
        raw_bl = (int(bytes[6].encode("hex"), 16) << 8) + int(bytes[7].encode("hex"), 16)
...
def main():
    ...
    board.wait(200)
    # Flash the LED so that we know we can step on.
